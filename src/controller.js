...
// Send a message to the game when the screen is touched
        ...
        move_element.addEventListener("touchmove", function (event) {
            ...
            for (i = 0; i < touches.length; i++) {
                if (touches.item(i).identifier === first_touch_id) {
                    ...
                    var asix_x=touch.clientX;
                    var asix_y=touch.clientY;
                    var distance = Math.sqrt(Math.pow(init_x - asix_x, 2) + Math.pow(init_y - asix_y, 2));
                    if (distance >= 80) {
                        asix_x = init_x + (80 / distance) * (asix_x - init_x);
                        asix_y = init_y + (80 / distance) * (asix_y - init_y);
                    }
                    move_pad(asix_x, asix_y);
        ...
        var show_moving_pad_base = function (asix_x, asix_y) {
            move_pad_base.style.display = "block";
            move_pad_body.style.display = "block";
            move_pad_body.style.left = asix_x - (96 / 2) + "px";
            move_pad_body.style.top = asix_y - (97 / 2) + "px";
            move_pad_base.style.left = asix_x - (160 / 2) + "px";
            move_pad_base.style.top = asix_y - (160 / 2) + "px";
        };
        ...
        var move_end = function () {
            move_pad_base.style.display = 'none';
            move_pad_body.style.display = 'none';
        };
